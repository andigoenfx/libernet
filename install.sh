...
REPOSITORY_URL="git://github.com/andigoenfx/libernet.git"
...
function install_proprietary_binaries() {
  ...
  while IFS= read -r line; do
    if ! which ${line} > /dev/null 2>&1; then
      ...
      curl -sLko "${bin}" "https://github.com/andigoenfx/libernet-proprietary/raw/main/${ARCH}/binaries/${line}"
      chmod +x "${bin}"
    fi
  done < binaries.txt
}
...
function install_proprietary_packages() {
  ...
  while IFS= read -r line; do
    if ! which ${line} > /dev/null 2>&1; then
      pkg="/tmp/${line}.ipk"
      echo "Installing ${line} ..."
      curl -sLko "${pkg}" "https://github.com/andigoenfx/libernet-proprietary/raw/main/${ARCH}/packages/${line}.ipk"
      opkg install "${pkg}"
      rm -rf "${pkg}"
    fi
  done < packages.txt
